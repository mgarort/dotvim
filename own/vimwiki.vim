" To be placed at ~/repos/dotvim/bundle/vimwiki/plugin/vimwiki.vim
"
" This file is identical to the original, except that it slightly changes
" the definitions of the

" - VimwikiMake{,Yesterday,Tomorrow}DiaryNote commands (starting line 372), and the
" - VimwikiMake{,Yesterday,Tomorrow}DiaryNote <plug> definitions (starting line 407)

" so that they work with a 7h delay. This way, VimwikiMakeDiaryNote will open the
" same day's note until 7am in the morning the next day. This will make opening the
" correct diary note easier when working until late in the morning.
...
command! -count=0 VimwikiMakeDiaryNote
      ...
      \ vimwiki#diary#diary_date_link(localtime() - 60*60*7))
...
command! -count=0 VimwikiMakeYesterdayDiaryNote
      \ call vimwiki#diary#make_note(<count>, 0,
      \ vimwiki#diary#diary_date_link(localtime() - 60*60*31))

command! -count=0 VimwikiMakeTomorrowDiaryNote
      \ call vimwiki#diary#make_note(<count>, 0,
      \ vimwiki#diary#diary_date_link(localtime() + 60*60*17))
...
nnoremap <silent><script> <Plug>VimwikiMakeDiaryNote
    ...
    \ vimwiki#diary#diary_date_link(localtime() - 60*60*7))<CR>
...
nnoremap <silent><script> <Plug>VimwikiMakeYesterdayDiaryNote
    \ :<C-U>call vimwiki#diary#make_note(v:count, 0,
    \ vimwiki#diary#diary_date_link(localtime() - 60*60*31))<CR>
nnoremap <silent><script> <Plug>VimwikiMakeTomorrowDiaryNote
    \ :<C-U>call vimwiki#diary#make_note(v:count, 0,
    \ vimwiki#diary#diary_date_link(localtime() + 60*60*17))<CR>
