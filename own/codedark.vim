...
" s:cdLeftDark is the color used for the color group StatusLineNC, which is the
" inactive status line. s:cdLeftMid is the color used for StatusLine, which is
" the active status line. The problem I have is that the inactive status line
" is currently too dark, so I don't see where one window ends and the next
" begins. Ideally, I would just change StatusLineNC to cd:LeftDarkMid.
" However, when the active and inactive colors of the status line are the
" same, Vim fills the status line with caret characters (^), which is really
" ugly (see https://vi.stackexchange.com/questions/15873/carets-in-status-line).
" The solution is to create a customized s:LeftDarkMid, called
" s:LeftDarkMyMid, which is the same as s:LeftDarkMid for the terminal, which
" is what we care about, but is different for the GUI, which we dont' care
" about. This way Vim doesn't recognize StatusLine and StatusLineNC to be the
" same color, because their GUI color is not the same, so it doesn't fill the
" status line with carets. See the StackExchange question for a better
" explanation.
let s:cdLeftMyMid = {'gui': '#252526', 'cterm': s:cterm03, 'cterm256': '237'}
...
call <sid>hi('StatusLineNC', s:cdFront, s:cdLeftMyMid, 'none', {})
